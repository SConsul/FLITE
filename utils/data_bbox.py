...
class DatasetFromClipPathsBBox(Dataset):
    def __init__(self, clip_paths, with_labels, bbox_path="../dataset/orbit_clutter_bounding_boxes"):
        '''
        clip_paths is location of file
        clip_bbox must be coordinates
        '''
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        for frame_path in self.clip_paths[index]: 
            ...
            if self.bbox_path is not None: 
                # self.clip_paths[index][0] = "..dataset/orbit_benchmark_244/train/USER/class/clean/TAG/IMG_NAME"
                name_split_list = self.clip_paths[index][0].split('/')
                #bbox_path/train_annotations/TAG.json
                json_path = os.path.join(self.bbox_path,name_split_list[2]+"_annotations",name_split_list[-2]+".json")
                with open(json_path, 'r') as f:
                    bbox_data=f.read()    
                self.bbox = json.loads(bbox_data)
                bbox_list.append(self.bbox[self.clip_paths[index]]['object_bounding_box'])
        if self.bbox_path is not None and self.with_labels:
            return torch.stack(clip, dim=0), self.clip_labels[index], bbox_list
        elif self.with_labels:
            return torch.stack(clip, dim=0), self.clip_labels[index]
        elif self.bbox_path is not None:
            return torch.stack(clip, dim=0), bbox_list
        else:
...
def get_clip_loader_bbox(clips, batch_size, with_labels=False):
    if isinstance(clips[0], np.ndarray):
        clips_dataset = DatasetFromClipPathsBBox(clips, with_labels=with_labels)
